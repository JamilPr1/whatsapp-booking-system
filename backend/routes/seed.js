...
// Seed demo data endpoint (for testing - should be protected in production)
router.post('/demo-data', async (req, res) => {
  try {
    ...
    const totalEntries = 1 + 1 + 1 + clients.length + services.length + createdBookings.length;

    res.json({
      success: true,
      message: 'Demo data seeded successfully!',
      summary: {
        admin: admin ? 'Created/Exists' : 'Failed',
        provider: provider ? 'Created/Exists' : 'Failed',
        driver: driver ? 'Created/Exists' : 'Failed',
